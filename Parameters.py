...
tracker_state = torch.reshape(torch.tensor([[50.], [50.], [80.], [-3.], [4.], [4.]]), (m,1))

### Initial State 1ST and 2ND Moments ###
m1x_0 = torch.reshape(torch.tensor([[10.], [10.], [90.], [-3.], [4.], [4.]]), (m,1))
m2x_0 = torch.eye(m) * torch.tensor([2**2, 2**2, 2**2, 0.05**2, 0.05**2, 0.05**2])
...
def dummy_h(x,tracker_state,jacobian=False, type="dummy"):
...
def h(x, tracker_state,jacobian = False):
    ...
    if jacobian:
        ...
        jac_h = torch.autograd.functional.jacobian(observe_h, x).view(n, m)
        return observe_h(x), jac_h
    else:
        return observe_h(x)
